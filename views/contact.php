    ...
    <div class="grid grid-2" style="gap:36px; align-items:start;">
      ...
      <div>
        <div class="glass-card" style="padding:28px; margin-bottom:20px;">
          ...
          <?php
            // The WhatsApp contact is the head-office number, read from the super admin's own
            // profile. One place to change it, and it follows whoever actually answers: a number
            // typed into a settings box goes stale the moment that person changes phone. With no
            // number on file nothing is shown, rather than a link that goes nowhere.
            $waNumber = '';
            try {
                ...
                // wa.me wants country code and digits only. The SMS normaliser already knows how to
                // turn a local number into that, and is the same routine the member forms trust.
                $waNumber = $waRaw !== ''
          ...
          <?php if ($waNumber !== ''): ?>
            <p style="margin:12px 0 0;">
              <a href="https://wa.me/<?= e($waNumber) ?>?text=<?= e(rawurlencode('Hello ' . ($s['site_title'] ?? '') . ', I found you on your website.')) ?>"
                 target="_blank" rel="noopener"
                 aria-label="Chat with us on WhatsApp"
                 style="display:inline-flex;align-items:center;gap:9px;color:var(--gold-soft);font-weight:600;text-decoration:none;">
