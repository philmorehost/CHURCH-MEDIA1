...
final class ReadingReminder
{
    ...
    public static function targets(PDO $pdo): array
    {
        $sql = "SELECT d.id AS device_id, d.token, m.id AS member_id, m.reading_plan_id,
                ...
                JOIN reading_plans p ON p.id = m.reading_plan_id AND p.is_published = 1
                WHERE d.token <> ''
    ...
    /** Every device bound to a member, so an empty run can say so rather than looking like a bug. */
    private static function devices(PDO $pdo): array
    {
        $sql = "SELECT id FROM device_tokens WHERE member_id IS NOT NULL AND token <> '' LIMIT " . self::BATCH_LIMIT;
     ...
     * can change the row, and the loser is told so by rowCount() rather than by luck.
     */
    private static function claim(PDO $pdo, int $memberId): bool
    {
        $stmt = $pdo->prepare(
            'UPDATE members SET reading_reminded_on = CURDATE()
             WHERE id = ? AND (reading_reminded_on IS NULL OR reading_reminded_on <> CURDATE())'
        );
    ...
    /** How many members are on a published plan, for the admin screen and the worker's --status. */
    public static function audienceSize(): int
    {
        ...
        return (int) Database::getInstance()->getConnection()
            ->query('SELECT COUNT(*) FROM members m JOIN reading_plans p ON p.id = m.reading_plan_id AND p.is_published = 1 WHERE m.is_suspended = 0')
            ->fetchColumn();
    }
}
