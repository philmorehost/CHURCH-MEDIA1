...
The GitHub Actions workflow (`.github/workflows/main.yml`) builds a **release APK and AAB** for this app on every push to `main` — and for `LIVINGWORD/` in the same run — uploading them as artifacts (`rccglp63-apk`, `rccglp63-aab`). Publishing to Google Play is done manually:

1. Open **Actions → "Build Android apps" → latest run**.
2. Download the **`rccglp63-aab`** artifact.
...
**Signing.** The workflow signs with the release keystore only when the
`ANDROID_KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS` and `KEY_PASSWORD` repository
secrets are set. Without them the build still succeeds, signed with the **debug key** —
installable for testing, but Play rejects it.
